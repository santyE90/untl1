import { ArrowUpRight, Bell, BookOpen, CalendarDays, ClipboardCheck, FileText, GraduationCap, MapPin, Presentation, ReceiptText, Repeat2, WalletCards } from "lucide-react";
...
export function CalendarItemCard({ compact = false, item, open = false, timeZone }: { compact?: boolean; item: CalendarItem; open?: boolean; timeZone: string }) {
  const Icon = item.sourceType === "native" ? CalendarDays : item.sourceType === "bill" ? ReceiptText : item.sourceType === "income" ? WalletCards : item.sourceType === "course_meeting" ? BookOpen : item.type === "quiz" ? ClipboardCheck : item.type === "project" || item.type === "presentation" ? Presentation : item.type === "midterm" || item.type === "final exam" ? GraduationCap : FileText;
  ...
  const body = <>
    <div className="flex min-w-0 items-start gap-2">
      <Icon aria-hidden className={cn("mt-0.5 size-4 shrink-0", item.sourceType === "bill" ? "text-financial-negative" : item.sourceType === "income" ? "text-financial-positive" : item.sourceType === "assessment" ? "text-destructive" : "text-primary")} />
  ...
  return <details className="group rounded-xl border bg-card shadow-xs" open={open || undefined}><summary className="cursor-pointer list-none p-4 [&::-webkit-details-marker]:hidden">{body}</summary><div className="space-y-3 border-t px-4 py-3 text-sm">
    ...
    <Link className="inline-flex min-h-10 items-center gap-1 font-semibold text-primary hover:underline" href={item.sourceUrl}>{item.isEditable ? "View event" : item.sourceType === "bill" || item.sourceType === "income" ? "View in Finance" : "View course"}<ArrowUpRight className="size-4" /></Link>
  </div></details>;
}
