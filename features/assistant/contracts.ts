...
export const assistantMutationResponseSchema = z.discriminatedUnion("ok", [
  z.object({ ok: z.literal(true), operation: assistantMutationNameSchema, task: z.object({ id: z.string().uuid(), title: z.string(), status: z.string(), priority: z.string(), due_date: z.string().nullable(), due_at: z.string().nullable(), estimated_effort_minutes: z.number().nullable(), assessment_id: z.string().uuid().nullable(), goal_id: z.string().uuid().nullable(), updated_at: z.string() }).passthrough(), references: z.array(assistantReferenceSchema).max(assistantLimits.maxReferences) }).strict(),
...
export const assistantStreamEventSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("confirmation"), token: z.string().min(32).max(160), expiresAt: z.string().datetime(), preview: z.object({ operation: assistantMutationNameSchema, actionLabel: z.string().min(1).max(80), taskTitle: z.string().min(1).max(200), changes: z.array(z.object({ label: z.string().min(1).max(80), before: z.string().max(10000).optional(), after: z.string().max(10000) }).strict()).min(1).max(12) }).strict() }).strict(),
