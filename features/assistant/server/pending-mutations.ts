...
export async function consumePendingMutation(token: string, context: AuthenticatedAppContext, now = Date.now()): Promise<ServiceResult<{ entity: TaskMutationResult | CalendarEventMutationResult; operation: AssistantMutationName }>> {
  sweep(now);
  const entry = pending.get(token);
  ...
  pending.delete(token);
  let result: ServiceResult<TaskMutationResult | CalendarEventMutationResult>;
  ...
  else result = await updateNativeCalendarEvent(entry.mutation.eventId, entry.mutation.input, context, { expectedUpdatedAt: entry.mutation.expectedUpdatedAt, preserveReminders: true });
