...
export async function saveTask(formData: FormData) {
  ...
  if (data.goalId) {
    const { data: goal, error } = await supabase.from("goals").select("id").eq("id", data.goalId).eq("user_id", user.id).is("archived_at", null).maybeSingle();
    if (error || !goal) fail("The related goal is unavailable.");
  }
  ...
  const row = { title: data.title, description: data.description, status: data.status, priority: data.priority, due_date: dueDate, due_at: dueAt, estimated_effort_minutes: data.estimatedEffortMinutes || null, assessment_id: data.assessmentId || null, goal_id: data.goalId || null };
