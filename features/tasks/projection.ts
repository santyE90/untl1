...
export function taskToCalendarItem(task: TaskWithContext): CalendarItem | null {
  ...
  return {
    ...
    description: `${task.status.replaceAll("_", " ")}${task.assessment ? ` · ${task.assessment.courseCode} · ${task.assessment.name}` : ""}${task.description ? ` · ${task.description}` : ""}`,
    location: null,
    amount: null,
    currency: null,
    isEditable: false,
    ...
    recurrence: null,
    reminderOffsets: [],
    metadata: { priority: task.priority, status: task.status, assessmentId: task.assessment_id },
  };
}
