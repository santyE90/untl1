...
type Defaults = { id?: string; title?: string; description?: string | null; status?: string; priority?: string; dueKind?: string; dueDate?: string; dueLocal?: string; estimatedEffortMinutes?: string; assessmentId?: string | null; goalId?: string | null };
const input = "mt-1 h-10 w-full rounded-lg border bg-card px-3 text-sm";

export function TaskForm({ assessments, goals, defaults = {} }: { assessments: AssessmentOption[]; goals: GoalOption[]; defaults?: Defaults }) {
  ...
  return <form action={saveTask} className="grid gap-3 sm:grid-cols-2">
    ...
    <label className="text-sm font-medium sm:col-span-2">Related goal<select className={input} defaultValue={defaults.goalId ?? ""} name="goalId"><option value="">None</option>{goals.map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select></label>
