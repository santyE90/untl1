...
The Calendar default-view editor is centralized in `/settings`; `/calendar/settings` remains the archive/restore surface and links back to the centralized preference. The Assistant section is informational because model, cost, confirmation, rate, and security controls remain application-owned; it explains ephemeral conversations, bounded authenticated tools, confirmation, and timezone context. Account & Data displays read-only email, profile creation date, and preference summaries. Email/password changes, account deletion, portability/export, notifications, integrations, billing, and admin/developer controls remain deferred. Phase 9 itself added no migration or new preference column.

## 29. Focused usability and CRUD corrections

The post-Phase-9 correction pass keeps domain ownership unchanged: responsive forms call Server Actions, actions derive `AuthenticatedAppContext`, and ordinary RLS-bound Supabase clients or narrowly scoped authenticated RPCs perform mutations. A shared Base UI toast presenter consumes conventional `success`/`error` redirects, announces results accessibly, auto-dismisses, and removes feedback parameters from the URL. Destructive actions use a shared alert-dialog confirmation with explicit permanent-delete wording; raw database/provider errors are not exposed.

School terms and courses retain archive-first history but may be permanently deleted through dedicated functions only when empty. Course meetings remain normalized as one authoritative weekday row per pattern. `replace_course_meetings(uuid,jsonb)` validates ownership and replaces the complete bounded schedule in one PostgreSQL transaction, allowing Monday and Wednesday to have independent times, types, and rooms while preventing partial writes. Calendar continues to project these School-owned rows; no native events are duplicated. Assessments and resources remain editable/archiveable, and archived resources join the existing parent-aware restore surface.

Finance account metadata (name, type, institution, optional credit limit, and net-worth inclusion) is editable. Currency, opening balance, and opening date remain immutable in the editor because they define the auditable ledger baseline. `delete_unused_finance_account` deletes only an owned account with no transactions, transfers, or recurring references; otherwise the user is directed to archive. Recurring bills and income are editable and confirmation-deletable. Deleting a template atomically clears only its optional source reference from posted transactions before removing the template, so ledger amount, date, category, and account history remain unchanged. Custom categories are editable and archive/restore capable. Monthly budgets retain their editable upsert semantics and may be confirmation-deleted through `delete_finance_budget`; category allocations cascade with the plan while ledger transactions and derived actual spending remain untouched.

Tasks and Goals retain soft deletion and now expose restore paths; native Calendar and School already retain archive restoration. This is deliberate: permanent deletion is not universal, and records with historical or cross-domain meaning remain archive-first.

`profiles.theme_preference` stores `system`, `light`, or `dark`, is owner-update-only under existing profile RLS, and is loaded through `AuthenticatedAppContext`. A small pre-hydration local preference script minimizes theme flash, while authenticated profile state remains authoritative across devices and System listens for OS color-scheme changes. The semantic token contract supplies both light and dark palettes. Timezone storage remains canonical IANA text; Settings exposes the runtime-supported broad list with recognizable city and region labels while server validation accepts only valid IANA zones.

The migrations `20260828000100_usability_crud_theme.sql` and `20260828000200_safe_budget_delete.sql` add the theme column and bounded School/Finance RPCs. Both hosted migrations were applied through the linked CLI and `types/database.ts` was regenerated from the linked schema. No service-role client, user-supplied owner ID, Calendar duplication, new feature module, or Assistant capability was introduced.
