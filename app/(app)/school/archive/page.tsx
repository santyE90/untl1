...
export default async function SchoolArchivePage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <div className="space-y-7"><header><Link className="inline-flex items-center gap-2 text-sm font-semibold text-primary" href="/school"><ArrowLeft className="size-4" />School</Link><h1 className="mt-3 text-3xl font-bold">School archive</h1><p className="mt-1 text-sm text-muted-foreground">Restore parents before children. Restoring a parent never clears a child’s own archive state.</p></header>{query.error ? <p className="rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{query.error}</p> : null}{query.success ? <p className="rounded-lg bg-success/10 p-3 text-sm text-success">{query.success}</p> : null}<section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4"><ArchiveList title="Terms" empty="No archived terms." rows={data.terms.map((term) => ({ id: term.id, label: term.name, detail: `${term.start_date} – ${term.end_date}`, type: "term", disabled: false }))} /><ArchiveList title="Courses" empty="No archived courses." rows={data.courses.map((course) => { const term = data.allTerms.get(course.term_id); return { id: course.id, label: `${course.code} · ${course.name}`, detail: term?.name ?? "Unknown term", type: "course", disabled: Boolean(term?.archived_at) }; })} /><ArchiveList title="Assessments" empty="No archived assessments." rows={data.assessments.map((assessment) => { const course = data.allCourses.get(assessment.course_id); const term = course ? data.allTerms.get(course.term_id) : null; return { id: assessment.id, label: assessment.name, detail: course ? `${course.code} · ${course.name}` : "Unknown course", type: "assessment", disabled: Boolean(course?.archived_at || term?.archived_at) }; })} /><ArchiveList title="Resources" empty="No archived resources." rows={data.resources.map((resource) => { const course = data.allCourses.get(resource.course_id); const term = course ? data.allTerms.get(course.term_id) : null; return { id: resource.id, label: resource.label, detail: course ? `${course.code} · ${course.name}` : "Unknown course", type: "resource", disabled: Boolean(course?.archived_at || term?.archived_at) }; })} /></section></div>;
}
